...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  // For development, create a mock session if auth is disabled
  const mockSession = process.env.NEXT_PUBLIC_DISABLE_AUTH === 'true' ? {
  ...
  useEffect(() => {
    async function checkSubscription() {
      ...
      try {
        const response = await apiClient.getSubscriptionStatus(currentSession.user.id);
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-gray-100">
      ...
      <div className={cn(
        ...
        {/* Sidebar header */}
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200/60 bg-gradient-to-r from-blue-500/5 to-indigo-500/5">
          {!sidebarCollapsed && (
          ...
          <div className="px-6 py-3 border-b border-gray-200/60">
            <div className="flex items-center space-x-2 text-xs">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-blue-600 font-medium">RAG Active</span>
              </div>
            ...
            {navigation.map((item) => {
              ...
              const linkContent = (
                <>
                  ...
                  {/* Blue accent strip */}
                  {isActive && hasAccess && (
                    <div className="absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-blue-500 to-indigo-600 rounded-r-full shadow-sm" />
                  )}
        ...
        <div className="lg:hidden">
          <div className="flex items-center justify-between h-16 px-4 bg-white/95 backdrop-blur-xl border-b border-gray-200/60">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-lg font-bold text-gray-900">MLOps Finance</span>
            </div>
            <div className="w-10" />
          </div>
        </div>
